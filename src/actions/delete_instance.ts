...
module.exports = {
	...
	callback: function (main, ctx) {
		if (ctx.senderSelectedInstance != null) {
			if (
				ctx.text ==
				ctx.senderMessages.delConfirmStr + ctx.sender.selected
			) {
				ctx.sender.menu = "";
				...
				main.linkings = main.linkings.filter(function (linking) {
					...
					if (
						linking.instance.id != ctx.sender.id ||
					...
					ctx.respondChat(
						ctx.senderMessages.linkingDestroyed.replace(
							"<linking>",
							linking.name
						),
						ctx.opt
					...
					main.bot.sendNewMessage(
						linking.groupid,
						ctx.senderMessages.serverUnlinked,
						ctx.opt
					);
					linking.Unlink();
					return false;
				...
				main.instances = main.instances.filter(function (instance) {
					return (
						instance.id != ctx.senderSelectedInstance.id ||
