...
module.exports = {
	...
	callback: function (main, ctx) {
		...
		let none = main.commands.some(function (obj, i) {
			...
			if (isAvailable) {
				let between =
					ctx.senderMessages["cmd_" + obj.description] || "empty";
				let overlen =
					obj.usage.length + between.length - (info ? 37 : 39);
				if (overlen > 0)
					between =
						between.substring(0, between.length - overlen - 2) +
						"..";
				//
				let ifo = "";
				if (info) {
		...
		msgtxt += msgtxt2;
		if (info)
			msgtxt += ctx.isGroup ? +cmsgs.commandsGroup : cmsgs.commandsChat;
		msgtxt += cmsgs.commandsDetail;
		ctx.opt.parse_mode = "html";
		if (!ctx.isGroup)
			ctx.opt.reply_markup.inline_keyboard = [
				[Utils.getCmdBtn("help", ctx.senderMessages)],
			];
