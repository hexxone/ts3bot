...
module.exports = {
	...
	callback: function (main, ctx) {
		...
		if (ctx.isGroup) {
			...
			msg +=
				gmsgs.settings10 + Utils.nmToStr(grb.language, grb.notifymove);
			msg += "</code>";
			...
			if (
				...
				ctx.opt.reply_markup.keyboard = [
					["/menu"],
					[
						"/admin4all " + !grb.alladmin,
						"/setchatmode " +
							Utils.cmToStr(ctx.sender.language, nextChatMode),
					],
					secondLn,
					[
						"/showserver " + !grb.showservername,
						"/ignorebots " + !grb.ignorebots,
					],
					["/silent " + !grb.silent, "/spamcheck " + !grb.spamcheck],
					[
						"/notifyjoin " + !grb.notifyjoin,
						"/notifymove " + ((grb.notifymove + 1) % 3),
					],
				];
