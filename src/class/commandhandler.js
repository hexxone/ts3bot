	...
	handle: function (self, ctx, mode, value) {
		return !self.commands.reduce(function (cont, obj) {
			...
			return obj.command.reduce(function (cont2, command) {
				...
				if (mode == 0 && command.toLowerCase() !== value.toLowerCase())
					return true;
				let exec = false;
				if (ctx.isGroup) {
					// sent in group?
					...
					if (obj.available > 1) {
						...
						if (
							obj.needslinking &&
							!(exec = ctx.groupBinding ? true : false)
						)
							self.bot.sendNewMessage(
								ctx.chatId,
								msgs.commandNotLinked,
								ctx.opt
							);
						...
						if (
							...
							!(exec =
								!ctx.groupBinding ||
								ctx.groupBinding.instance.id == ctx.sender.id ||
								ctx.groupBinding.alladmin)
						)
							self.bot.sendNewMessage(
								ctx.chatId,
								msgs.commandForbidden,
								ctx.opt
							);
					...
					if (
						obj.available === 0 &&
						ctx.sender.id == self.developer_id
					)
						exec = true;
					// is group command ?
						...
						self.bot.sendNewMessage(
							ctx.chatId,
							msgs.commandErrChat2,
							ctx.opt
						);
					else {
						if (obj.needsselected) {
							if (
								!(exec = ctx.senderSelectedInstance
									? true
									: false)
							) {
								if (ctx.senderInstances.length > 0)
									self.bot.sendNewMessage(
										ctx.chatId,
										ctx.senderMessages.commandNoSelect,
										ctx.opt
									);
								else
									self.bot.sendNewMessage(
										ctx.chatId,
										ctx.sender.agreement
											? msgs.commandNoAdded
											: msgs.commandNoTOS,
										ctx.opt
									);
				...
				if (exec) {
					console.log(
						"COMMAND: " + command + " by: " + ctx.sender.id
					);
					if (!ctx.isGroup && !ctx.isReply)
						self.bot.deleteMessage(ctx.chatId, ctx.msg.message_id);
