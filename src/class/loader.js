	...
	Methods: {
		...
		loadData: function (t) {
			...
			try {
				...
				let data =
					decipher.update(crypted[1], "hex", "utf8") +
					decipher.final("utf8");
				// parse to object
			...
			objj.linkings.forEach((lnk) => {
				...
				console.log(
					"Loaded linking: " + instt.id + " name: " + lnkk.name
				);
			});
			...
			objj.deeplinking.forEach((keyset) => {
				...
				console.log(
					"Loaded deeplink: " + instt.id + " name: " + lnkk.name
				);
			});
		...
		saveData: function () {
			...
			this.Parent.instances.forEach((inst) =>
				objj.instances.push(inst.Export())
			);
			this.Parent.linkings.forEach((lnk) =>
				objj.linkings.push(lnk.Export())
			);
			this.Parent.deeplinking.forEach((val, key) =>
				objj.deeplinking.push({ k: key, v: val.Export() })
			);
			this.Parent.groupnames.forEach((val, key) =>
				objj.groupnames.push({ k: key, v: val })
			);
			...
			let crypted =
				cipher.update(txtt, "utf8", "hex") + cipher.final("hex");
		...
		loadModules: function () {
			this.loadFolder(
				this.Parent.actionsPath,
				this.Parent.actions,
				" actions."
			);
			this.loadFolder(
				this.Parent.commandsPath,
				this.Parent.commands,
				" commands."
			);
			this.loadFolder(
				this.Parent.languagesPath,
		...
		loadModule: function (array, file) {
			try {
				...
				console.log(
					"Couldnt remove from require cache (new file?): " + ex
				);
			}
			try {
				// require & find module
				...
				if (!mod || !mod.id)
					throw new Exception("reload did not include id: " + file);
				let ix = -1;
		...
		createLinkingFromData(instt, lnk) {
			...
			for (let usrid of lnk.userids)
				for (let usrr of this.Parent.users)
					if (usrid == usrr.id) lnkk.CheckAddUser(usrr);
